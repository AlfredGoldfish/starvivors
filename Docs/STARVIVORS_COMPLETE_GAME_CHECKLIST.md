...
- [~] Enemy telegraphs are readable.
  ...
  - Evidence: 2026-05-30 Impact Bomber validation promoted `impact-bomber` as the active live validation enemy while keeping Scout, Wedge Striker, Hex Tank, and Reactor Drone pinned as regression harnesses; `testHarness=impactBomberPhase` verifies solo Impact spawning, the exact Scout+Wedge+Tank+Reactor+Impact mix, approach outside trigger range, proximity-fuse countdown start in a non-contact proximity band, countdown cancel/reset when the player exits trigger range, fuse restart on re-entry, blast-ring feedback on completed detonation, inside/escaped blast hull behavior, normal reward cleanup after self-detonation, body-contact HP safety, and square danger-mark visuals distinct from Reactor Drone.
