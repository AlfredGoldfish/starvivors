...
Phase 16 starts the SVG/vector asset pipeline by using shared enemy visual data as the first host for a broader Starvivors Asset Forge.
...
- Enemy textures route through the Forge renderer by converting existing `EnemyVisualDefinition` data into Forge assets.
- The standalone lab panel has been removed; future Forge promotion should happen through main-game or dedicated asset tooling.
...
- Enemy squads, behavior testing, diagnostics, and preset import/export need direct main-game replacement if this workflow is resumed.
- AI briefs and promotion reports should include the Neon-Forward Salvagepunk style guide so AI-generated assets inherit the visual direction.
